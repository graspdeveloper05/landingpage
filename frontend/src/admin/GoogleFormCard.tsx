...
export function GoogleFormCard() {
  ...
  // Folded by default: it is set up once, and the attendee list below is
  // what this page is opened for.
  ...
  return (
    <AdminCard className="mb-6">
      ...
      >
        <span>
          <span className="block text-[0.88rem] font-semibold text-navy-950">
            Google Form and registration settings
          </span>
          <span className="mt-0.5 block text-[0.76rem] text-slate">
            {saved === null
              ? 'Loading…'
              : [
                  saved.mode === 'google'
                    ? 'People register on the Google Form'
                    : 'People register on this website',
                  saved.url ? 'form connected' : 'no Google Form',
                  saved.webapp ? 'website registrations added to the form' : null,
                ]
                  .filter(Boolean)
                  .join(' · ')}
          </span>
      ...
      {open && (
        <div className="mt-5 border-t border-[#DDDCD8] pt-5">
          <fieldset className="mb-5">
            <legend className="text-[0.78rem] font-semibold text-navy-900">
          ...
          {saved && !changed && !error && (
            <p className="mt-2 text-[0.78rem] text-slate">
              {saved.url ? (
                <>
                  <span className="font-semibold text-green-700">Connected</span> · all{' '}
                  {saved.matched} questions matched ·{' '}
                  {saved.mode === 'google'
          ...
          <div className="mt-5 border-t border-[#DDDCD8] pt-4">
            ...
            {script && (
              <>
                ...
                <p className="mt-1 text-[0.72rem] text-slate">
                  The key in this script lets the form add registrations here, so send it to the
                  form's owner rather than posting it publicly. “Issue a new key” stops an old copy
                  working.
                </p>
              </>
            )}
            ...
            {saved?.webapp && webapp.trim() === saved.webapp && !webappError && (
              <p className="mt-2 text-[0.78rem] text-slate">
                <span className="font-semibold text-green-700">Connected</span> · registrations made
                here are added to the Google Form
              </p>
            )}
          </div>
        </div>
